...
namespace Unigram
{
	...
	{
		public float MajorX { get; set; }
		public float MajorY { get; set; }
		public float MinorX { get; set; }
		public float MinorY { get; set; }
		...
		public void PaintCoordinateSystem(Transformer transformer, Graphics g)
		{
			...
			for (double xBar = Math.Floor(transformer.XMin / this.MajorX) * this.MajorX; xBar <= transformer.XMax; xBar += this.MajorX)
			{
				float x = transformer.MathToPixel(new PointF((float)xBar, 1)).X;
				g.DrawLine(majorPen, x, 0, x, transformer.VPHeightPX);
				if (xBar != 0.0f) {
					g.DrawString(xBar.ToString("0.######"), axisFont, textBrush, x, Math.Min(Math.Max(zero.Y, 0), transformer.VPHeightPX - 18));					
				}
			}

			for (double yBar = Math.Floor(transformer.YMin / this.MajorY) * this.MajorY; yBar <= transformer.YMax; yBar += this.MajorY)
			{
				float y = transformer.MathToPixel(new PointF(1, (float)yBar)).Y;
				g.DrawLine(majorPen, 0, y, transformer.VPWidthPX, y);
				String text = yBar.ToString("0.######");
				if (yBar != 0.0f) {
					g.DrawString(text, axisFont, textBrush, Math.Min(Math.Max(zero.X, 0), transformer.VPWidthPX - text.Length * 12), y);
				}
			}
